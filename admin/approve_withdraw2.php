...
if(!isset($_SESSION['loggedin_a'])){
    ...
    if(isset($_POST['submit'])){
        ...
        $sql="SELECT * FROM `withdraw` WHERE win='$win' && status='pending Approval' ";
                            ...
                            while($row2 = mysqli_fetch_array($result2)) {
                                if($row['amount'] > $row2['Total_earnings']){
                                    ...
                                    echo "<script>alert('insufficient Funds')
                                                location.href='pending_withdraw_man.php'</script>";
                                }
                                else{
                                ...
                                $sql4  = "UPDATE users SET Total_earnings='$newearnings' WHERE unique_id='$row[uin]'";
                                            ...
                                            if($result4) {
                                                ...
                                                location.href='pending_withdraw_man.php'</script>";
                                                $sql6  = "UPDATE history SET status='Approved' WHERE hin='$win'";
                                                ...
                                                // un-pend withdraw
                                            if($result6) {
                                                $sql8  = "UPDATE withdraw SET status='Approved' WHERE win='$win'";
